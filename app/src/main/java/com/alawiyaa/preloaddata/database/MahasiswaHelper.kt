...
import com.alawiyaa.preloaddata.database.DatabaseContract.MahasiswaColumns.Companion.NAMA
import com.alawiyaa.preloaddata.database.DatabaseContract.MahasiswaColumns.Companion.NIM
import com.alawiyaa.preloaddata.database.DatabaseContract.TABLE_NAME
...
class MahasiswaHelper(context: Context) {
    ...
    fun getAllData(): ArrayList<MahasiswaModel> {
        val cursor = database.query(TABLE_NAME, null, null, null, null, null, "${BaseColumns._ID} ASC", null)
        cursor.moveToFirst()
        ...
        if (cursor.count > 0) {
            do {
                ...
                mahasiswaModel.id = cursor.getInt(cursor.getColumnIndexOrThrow(BaseColumns._ID))
    ...
    fun getDataByName(nama: String): ArrayList<MahasiswaModel> {
        val cursor = database.query(TABLE_NAME, null, "$NAMA LIKE ?", arrayOf(nama), null, null, "${BaseColumns._ID} ASC", null)
        cursor.moveToFirst()
        val arrayList = ArrayList<MahasiswaModel>()
        var mahasiswaModel: MahasiswaModel
        if (cursor.count > 0) {
